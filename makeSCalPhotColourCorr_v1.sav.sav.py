...
if calcColorCorrK:
	...
	if prev:
		#make dummy for previous method
		k4P_Prev = {}
		k4E_Prev = {}
	for band in spireBands:
		#pipeline beam areas
		...
		if prev:
			# use previous method
			k4P_Prev[band] = OLD_hpXcalKcorr(freq0, freq, transm, \
			  BB=False, alpha=-1.0)
			k4E_Prev[band] = OLD_hpXcalKcorr(freq0, freq, transm, \
			  BB=False, alpha=-1.0, gamma=gamma)
	...
	if fixBeta == True:
		...
		for beta in bvect:
			...
			kPMW = Double1d(hpXcalKcorr(spireRefFreq["PMW"], freq, spireFilt["PMW"], 
			  ...
			  monoArea=beamMonoArea["PMW"]))
			# multiply by effective beam area
			kPMW[ = kPMW[0] * spireEffArea(freq, spireFiltOnly["PMW"],
