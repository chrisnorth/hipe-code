...
if calcRadialCorrBeam:
	...
	(beamVersion,beamVersionOld,calVersion)
	...
	for band in spireBands:
		...
		# get core beam
		beamComb=beamProfs.getCoreCorrectionTable().getColumn(band).data
		# get const beam
...
if calcSpireEffFreq:
	...
	if calcRadialCorrBeam:
		...
		beamProfs.meta['freqEffPsw'].double=spireEffFreq['PSW']/1.e9 #in GHz
		beamProfs.meta['freqEffPmw'].double=spireEffFreq['PMW']/1.e9 #in GHz
		beamProfs.meta['freqEffPlw'].double=spireEffFreq['PLW']/1.e9 #in GHz
...
beamMonoArea={}
if simpleBeam:
	print '\nCalculating monochromatic beam areas for simple beam...'
	for band in spireBands:
		beamMonoArea[band] = spireMonoAreasSimple(freq, spireEffFreq[band],
		  spireAreaEffFreq[band], gamma)
else:
...
if calcColorCorrK:
	#-----------------------------------------------------------------------
	# copy old calibration products
	kCorrPsrcOld = kCorrPsrc.copy()
	kCorrPsrcOldVersion = kCorrPsrcOld.getVersion()
	kCorrExtdOld = kCorrExtd.copy()
	kCorrExtdOldVersion = kCorrExtdOld.getVersion()
	kCorrBeamOld = kCorrBeam.copy()
	...
	fluxConv = herschel.spire.ia.dataset.PhotFluxConv()
	...
	kCorrBeam.meta['beamPswSr'].double=beamAreaPipSr['PSW']
	kCorrBeam.meta['beamPmwSr'].double=beamAreaPipSr['PMW']
	kCorrBeam.meta['beamPlwSr'].double=beamAreaPipSr['PLW']
	kCorrBeam.meta['beamPswArc'].double=beamAreaPipArc['PSW']
	kCorrBeam.meta['beamPmwArc'].double=beamAreaPipArc['PMW']
	kCorrBeam.meta['beamPlwArc'].double=beamAreaPipArc['PLW']
	...
	if calcRadialCorrBeam:
		...
		print 'Updating pipeline beam areas in RadialCorrBeam version %s'
	...
	#-----------------------------------------------------------------------
	# write log file
	kCorrExtdLog=open(kCorrPsrcLogFile,'w')
	...
	if verbose:
		...
		print 'SPIRE K4 factors for point source: %6.4f, %6.4f, %6.4f' % \
		  (k4P["PSW"],k4P["PMW"],k4P["PLW"])
	
		# Print Spire K4 factors for extended source calibration for verification
		print 'SPIRE k4 (Total) factors for extended source: %6.4f, %6.4f, %6.4f' % \
		  (k4E_Tot["PSW"],k4E_Tot["PMW"],k4E_Tot["PLW"])
		print 'SPIRE K4 factors for extended source: %6.4f, %6.4f, %6.4f' % \
		  (k4E["PSW"],k4E["PMW"],k4E["PLW"])
	
		print 'SPIRE kPtoE conversion factor: %6.4f, %6.4f, %6.4f' %  \
		  (kPtoE["PSW"],kPtoE["PMW"],kPtoE["PLW"])

else:
	# use old values
	kCorrBeam = calibration.phot.getProduct('PhotColorCorrBeam')
...
if calcColorCorrAperture:
	...
	apCorrIncBGa.addColumn('alpha',Column(Float1d(alphaK)))
	apCorrIncBGa.addColumn('PSW',Column(Float1d(len(alphaK)),unit=Scalar.ONE,description=''))
	apCorrIncBGa.addColumn('PMW',Column(Float1d(len(alphaK)),unit=Scalar.ONE,description=''))
	apCorrIncBGa.addColumn('PLW',Column(Float1d(len(alphaK)),unit=Scalar.ONE,description=''))
	...
	for a in range(len(alphaK)):
		...
		for band in spireBands:
			...
			#apply colour correction and beam correction
			apPhotIncBG_x = apPhot_x.getTargetPlusSkyTotal() * \
			  kCorrExtd['alpha'][band].data[a] * \
			  kCorrBeam['alpha'][band].data[a]
			apPhotNoBG_x = apPhot_x.getTargetPlusSkyTotal() * \
			  kCorrExtd['alpha'][band].data[a] * \
			  kCorrBeam['alpha'][band].data[a]
			apPhotIncBG_x = apPhot_x.getTargetPlusSkyTotal()
			apPhotNoBG_x = apPhot_x.getTargetTotal()
			apCorrIncBGa[band].data[a]=effBeam_x['area']/apPhotIncBG_x
			apCorrNoBGa[band].data[a]=effBeam_x['area']/apPhotNoBG_x
...
#===============================================================================
#-------------------------------------------------------------------------------
kCorrHfi = calibration.phot.getProduct('ColorCorrHfi')
if calcColorCorrHfi:
	# copy old calibration product
	kCorrHfiOld = kCorrHfi.copy()
	...
	if fixBeta == True:
		...
		for temp in tvect:
			...
			if verbose:
				print 'temp=%f'%temp
				print '  k545[0]=%f, k857[0]=%f'%(k545[0],k857[0])
				print '  kPSW[0]=%f, kPMW[0]=%f, kPLW[0]=%f'%(kPSW[0],kPMW[0],kPLW[0])
				print '  k545[1]=%f, k857[1]=%f'%(k545[1],k857[1])
				print '  kPSW[1]=%f, kPMW[1]=%f, kPLW[1]=%f'%(kPSW[1],kPMW[1],kPLW[1])
		...
		for beta in bvect:
			...
			kPLW = Double1d(hpXcalKcorr(spireRefFreq["PLW"], freq, spireFilt["PLW"], 
			  ...
			  monoArea=beamMonoArea["PLW"]))
			# multiply by effective beam area
			kPLW = kPLW * spireEffArea(freq, spireFiltOnly["PLW"],
			...
			kPMW = Double1d(hpXcalKcorr(spireRefFreq["PMW"], freq, spireFilt["PMW"], 
			  ...
			  monoArea=beamMonoArea["PMW"]))
			# multiply by effective beam area
			kPMW = kPMW * spireEffArea(freq, spireFiltOnly["PMW"],
			...
			if verbose:
				print 'beta=%f'%beta
				print '  k545=%f, k857=%f'%(k545,k857)
				print '  kPSW=%f, kPMW=$f, kPLW=%f'%(kPSW[0],kPMW[0],kPLW[0])
		#
	...
	#write log file
	# write log file
	kCorrHfiLog=open(kCorrPsrcLogFile,'w')
	kCorrHfiLog.write('Extended Source Colour Correction Version %s\n'%version)
