...
#   3. Synthesises the effective beam profile and 2D beam map for given source
...
# 
#  Edition History
#  Chris North   - 18/Feb/2014 - 1.0: First version
#                                     Used for spire_cal_12_1
#===============================================================================
import os
scriptVersionString = "makeSCalPhotColorCorrK.py $Revision: 1.0 $"
...
apCorrAnaNoBG = herschel.spire.ia.dataset.PhotColorCorrAperture()
apCorrAnaIncBG = herschel.spire.ia.dataset.PhotColorCorrAperture()
apCorrAnaNoBG['alpha']=TableDataset(description='Aperture Correction Analytical results without background (Spectral Index)')
apCorrAnaIncBG['alpha']=TableDataset(description='Aperture Correction Analytical results including background (Spectral Index)')
apCorrAnaNoBG['alpha'].addColumn('alpha',Column(Float1d(alphaK)))
apCorrAnaIncBG['alpha'].addColumn('alpha',Column(Float1d(alphaK)))
for band in spireBands:
	#apCorrAna['alpha'].addColumn('omegaAp_%s'%band,Column(Float1d(len(alphaK))))
	#apCorrAna['alpha'].addColumn('omegaBG_%s'%band,Column(Float1d(len(alphaK))))
	#apCorrAna['alpha'].addColumn('omegaTot_%s'%band,Column(Float1d(len(alphaK))))
	apCorrAnaNoBG['alpha'].addColumn(band,Column(Float1d(len(alphaK))))
	apCorrAnaIncBG['alpha'].addColumn(band,Column(Float1d(len(alphaK))))
...
for band in spireBands:
	...
	apCorrAnaNoBG.meta['sizeAp_%s'%band]=DoubleParameter(sizeAp)
	apCorrAnaNoBG.meta['sizeBG_%s'%band]=DoubleParameter(sizeBG)
	apCorrAnaIncBG.meta['sizeAp_%s'%band]=DoubleParameter(sizeAp)
	apCorrAnaIncBG.meta['sizeBG_%s'%band]=DoubleParameter(sizeBG)
	...
	for a in range(len(alphaK)):
		...
		#apCorrAna['alpha']['omegaAp_%s'%band].data[a] = omegaAp
		#apCorrAna['alpha']['omegaBG_%s'%band].data[a] = omegaBG
		#apCorrAna['alpha']['omegaTot_%s'%band].data[a] = omegaTot

		apCorrAnaNoBG['alpha'][band].data[a] = \
			effBeam_x['area']/omegaAp
		apCorrAnaIncBG['alpha'][band].data[a] =  \
			effBeam_x['area']/(omegaAp - omegaBG*sizeAp/sizeBG)

# set FITS filenames
if outputCalDirTree:
	apCorrNoBGFits = java.io.File(r"%s//Phot//SCalPhotColorCorrAperture//SCalPhotColorCorrAperture_noBG_v%s.fits"%(directory, version))
	apCorrIncBGFits = java.io.File(r"%s//Phot//SCalPhotColorCorrAperture//SCalPhotColorCorrAperture_incBG_v%s.fits"%(directory, version))
	apCorrAnaNoBGFits = java.io.File(r"%s//Phot//SCalPhotColorCorrAperture//SCalPhotColorCorrAperture_Analytical_noBG_v%s.fits"%(directory, version))
	apCorrAnaIncBGFits = java.io.File(r"%s//Phot//SCalPhotColorCorrAperture//SCalPhotColorCorrAperture_Analytical_incBG_v%s.fits"%(directory, version))
else:
	...
	apCorrAnaNoBGFits = java.io.File(r"%s//SCalPhotColorCorrAperture_Analytical_noBG_v%s.fits"%(dataDir, version))
	apCorrAnaIncBGFits = java.io.File(r"%s//SCalPhotColorCorrAperture_Analytical_incBG_v%s.fits"%(dataDir, version))
...
fitsWriter = FitsArchive()
fitsWriter.rules.append(herschel.spire.ia.util.MetaDataDictionary.getInstance().getFitsDictionary())
fitsWriter.save(apCorrAnaNoBGFits.toString(), apCorrAnaNoBG)

fitsWriter = FitsArchive()
fitsWriter.rules.append(herschel.spire.ia.util.MetaDataDictionary.getInstance().getFitsDictionary())
fitsWriter.save(apCorrAnaIncBGFits.toString(), apCorrAnaIncBG)
