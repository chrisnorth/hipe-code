...
calibration = spireCal(jarFile=os.path.join(dataDir,'spire_cal_12_0.jar'))

# SPIRE aperture efficiency product from cal tree
apertureEfficiencyVersion = "1"
apertureEfficiency = calibration.phot.getProduct('ApertureEfficiency')
...
#calculate K4P and K4E parameters over alpha
alphaK=[-4.,-3.5,-3.,-2.5,-2.,-1.5,-1.,-0.5,0.,0.5,1.,1.5,2.,2.5,3.,3.5,4.,4.5,5.]
#make tables 
OldKConvPsrc=TableDataset(description='Point Source Conversion (Spectral Index)')
OldKCorrPsrc=TableDataset(description='Point Source Colour Correction (Spectral Index)')
OldKConvExtd=TableDataset(description='Extended Source Conversion (Spectral Index)')
OldKCorrExtd=TableDataset(description='Extended Source Colour Correction (Spectral Index)')
#add alpha columns
OldKConvPsrc.addColumn('alpha',Column(Float1d(alphaK)))
OldKCorrPsrc.addColumn('alpha',Column(Float1d(alphaK)))
OldKConvExtd.addColumn('alpha',Column(Float1d(alphaK)))
OldKCorrExtd.addColumn('alpha',Column(Float1d(alphaK)))
for band in ["PSW","PMW","PLW"]:
	#add band columns
	OldKConvPsrc.addColumn(band,Column(Float1d(len(alphaK))))
	OldKCorrPsrc.addColumn(band,Column(Float1d(len(alphaK))))
	OldKConvExtd.addColumn(band,Column(Float1d(len(alphaK))))
	OldKCorrExtd.addColumn(band,Column(Float1d(len(alphaK))))
for a in range(len(alphaK)):
	#PSW
	OldKConvPsrc['PSW'].data[a] = hpXcalKcorr(spireRefFreq[0], freq, \
	  spireFiltPSW, BB=False, alpha=alphaK[a])[0]
	OldKCorrPsrc['PSW'].data[a] = OldKConvPsrc['PSW'].data[a] / k4P_PSW
	OldKConvExtd['PSW'].data[a] = hpXcalKcorr(spireRefFreq[0], freq, \
	  spireFiltPSW, BB=False, alpha=alphaK[a],gamma=gamma)[0]
	OldKCorrExtd['PSW'].data[a] = OldKConvExtd['PSW'].data[a] / k4E_PSW

	#PMW
	OldKConvPsrc['PMW'].data[a] = hpXcalKcorr(spireRefFreq[1], freq, \
	  spireFiltPMW, BB=False, alpha=alphaK[a])[0]
	OldKCorrPsrc['PMW'].data[a] = OldKConvPsrc['PMW'].data[a] / k4P_PMW
	OldKConvExtd['PMW'].data[a] = hpXcalKcorr(spireRefFreq[1], freq, \
	  spireFiltPMW, BB=False, alpha=alphaK[a],gamma=gamma)[0]
	OldKCorrExtd['PMW'].data[a] = OldKConvExtd['PMW'].data[a] / k4E_PMW

	#PLW
	OldKConvPsrc['PLW'].data[a] = hpXcalKcorr(spireRefFreq[2], freq, \
	  spireFiltPLW, BB=False, alpha=alphaK[a])[0]
	OldKCorrPsrc['PLW'].data[a] = OldKConvPsrc['PLW'].data[a] / k4P_PLW
	OldKConvExtd['PLW'].data[a] = hpXcalKcorr(spireRefFreq[2], freq, \
	  spireFiltPLW, BB=False, alpha=alphaK[a],gamma=gamma)[0]
	OldKCorrExtd['PLW'].data[a] = OldKConvExtd['PLW'].data[a] / k4E_PLW
